...
var HCAChannelType$1 = HCAChannelType;
...
class HCAFrame {
    constructor(hca) {
        ...
        for (let i = 0; i < hca.format.channelCount; i++) {
            this.Channels.push(new HCAChannel({
                Type: channelTypes[i],
                CodedScaleFactorCount: channelTypes[i] == HCAChannelType$1.StereoSecondary
    ...
    static GetChannelTypes(hca) {
        ...
        if (hca.compDec.StereoBandCount == 0 || channelsPerTrack == 1) {
            return new Array(8).fill(HCAChannelType$1);
        }
        const Discrete = HCAChannelType$1.Discrete;
        const StereoPrimary = HCAChannelType$1.StereoPrimary;
        const StereoSecondary = HCAChannelType$1.StereoSecondary;
...
class HCAPacking {
    ...
    static PackFrame(frame, outBuffer) {
        ...
        for (let channel of frame.Channels) {
            ...
            if (channel.Type == HCAChannelType$1.StereoSecondary) {
    ...
    static ReadScaleFactors(channel, reader, hfrGroupCount, versionMajor) {
        ...
        if (channel.Type == HCAChannelType$1.StereoSecondary || hfrGroupCount <= 0 || versionMajor <= 2) {
...
class HCADecoder {
    ...
    static ApplyIntensityStereo(frame) {
        ...
        for (let c = 0; c < frame.Channels.length; c++) {
            if (frame.Channels[c].Type != HCAChannelType$1.StereoPrimary)
                continue;
            for (let sf = 0; sf < SubframesPerFrame; sf++) {
